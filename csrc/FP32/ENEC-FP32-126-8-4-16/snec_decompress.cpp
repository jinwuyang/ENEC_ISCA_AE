...
public:
    ...
    )
    {
        ...
        int FinalOtherSize = 32 + // 头
                            ...
                            (datablockSize / sizeof(uint32_t)) * 4 / 8 * datablockNum + // low bits
    ...
    __aicore__ inline void Process()
    {
        ...
        // AIV_WITH_BARRIER(ShiftLeft, mergeLocal, mergeLocal, (uint16_t)8, computeNum0 * 2 / 16);
        // AIV_WITH_BARRIER(ShiftRight, mergeLocal, mergeLocal, (uint16_t)8, computeNum0 * 2 / 16);
        ...
        // AIV_WITH_BARRIER(ShiftRight, mergeLocal, mergeLocal, (uint16_t)12, computeNum0);
...
private:
                                ...
                                )
    {
        ...
        if(accouterNum + outerNum >= computeNum)
        {
            ...
            AIV_WITH_BARRIER(ShiftRight, tempLocal1[16].template ReinterpretCast<int16_t>(), mergeLocal[accouterNum].template ReinterpretCast<int16_t>(), (int16_t)8, remainNum);
            ...
            AIV_WITH_BARRIER(ShiftRight, tempLocal1[16 + remainNum].template ReinterpretCast<int16_t>(), mergeLocal.template ReinterpretCast<int16_t>(), (int16_t)8, nextreadNum);
            accouterNum  = nextreadNum;
        }
        else {
            AIV_WITH_BARRIER(ShiftRight, tempLocal1[16].template ReinterpretCast<int16_t>(), mergeLocal[accouterNum].template ReinterpretCast<int16_t>(), (int16_t)8, outerNum);
            // if(i == 0){
    ...
    //     AIV_WITH_BARRIER(ShiftRight, tempLocal0[computeNum], tempLocal0[computeNum], (uint16_t)13, computeNum);
        ...
        // }
        AIV_WITH_BARRIER(Adds, tempLocal0[computeNum].template ReinterpretCast<int16_t>(), tempLocal0[computeNum].template ReinterpretCast<int16_t>(), (int16_t)126, computeNum);
