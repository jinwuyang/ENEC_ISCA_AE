...
public:
    ...
    __aicore__ inline void Process()
    {
        ...
        // AIV_WITH_BARRIER(ShiftLeft, mergeLocal[(cumulated_amount * 2 / 16)], mergeLocal[(cumulated_amount * 2 / 16)], (uint16_t)8, cumulated_amount * 2 / 16);
        // AIV_WITH_BARRIER(Or, mergeLocal, mergeLocal, mergeLocal[(cumulated_amount * 2 / 16)], cumulated_amount * 2 / 16);
...
private:
    ...
    {   // tempLocal0: e_inLocal(前半), mergeLocal(后半)
        ...
        AIV_WITH_BARRIER(Adds, tempLocal0.template ReinterpretCast<int16_t>(), tempLocal0.template ReinterpretCast<int16_t>(), (int16_t)(-126), computeNum);
        ...
        if(cumulated_amount + outerNum >= computeNum){
            ...
            // AIV_WITH_BARRIER(ShiftLeft, mergeLocal[(computeNum * 2 / 16)], mergeLocal[(computeNum * 2 / 16)], (uint16_t)8, computeNum * 2 / 16);
            // AIV_WITH_BARRIER(Or, mergeLocal, mergeLocal, mergeLocal[(computeNum * 2 / 16)], computeNum * 2 / 16);
