...
public:
    ...
    __aicore__ inline void Process()
    {
        ...
        AIV_WITH_BARRIER(DataCopy, e_output1[write_offset], mergeLocal, cumulated_amount / 8);
...
private:
                                ...
                                )// 3， 6
    {
        ...
        // e_inLocal(0) = 125;
        AIV_WITH_BARRIER(Adds, e_inLocal.template ReinterpretCast<int16_t>(), e_inLocal.template ReinterpretCast<int16_t>(), (int16_t)(-124), computeNum);
        ...
        // if(i == 0){
        //     DumpTensor(compareMask, 1 , 128 / sizeof(T));
        // }
        // AIV_WITH_BARRIER((Broadcast<half, 2, 0>), e_outLocal0.template ReinterpretCast<half>(), compareMask.template ReinterpretCast<half>(), dstShape_cmp, srcShape_cmp);// compareMask进行广播
        ...
        // if(i == 0){
        //     DumpTensor(compareMask[128 / sizeof(T)], 1, 128 / sizeof(T));
        // }
        ...
        // if(i == 0){
        //     DumpTensor(e_outLocal1, 1, 32);
        // }
        // AIV_WITH_BARRIER(ShiftRight, e_outLocal0, e_inLocal, (uint16_t)3, computeNum);
        // if(i == 0){
        //     DumpTensor(e_outLocal0, 1, computeNum);
        // }


        AIV_WITH_BARRIER(ShiftLeft, e_outLocal0, e_inLocal, (uint16_t)12, computeNum);
        AIV_WITH_BARRIER(ShiftRight, e_outLocal0, e_outLocal0, (uint16_t)12, computeNum);
        // if(i == 0){
        //     DumpTensor(e_outLocal0, 1, 64);
        // }

        AIV_WITH_BARRIER(ShiftLeft, e_outLocal0[computeNum / 2], e_outLocal0[computeNum / 2], (uint16_t)4, computeNum / 2);
        AIV_WITH_BARRIER(Or, e_outLocal0, e_outLocal0, e_outLocal0[computeNum / 2], computeNum / 2);

        AIV_WITH_BARRIER(ShiftLeft, e_outLocal0[(computeNum / 4)], e_outLocal0[(computeNum / 4)], (uint16_t)8, computeNum / 4);
        AIV_WITH_BARRIER(Or, e_outLocal0, e_outLocal0, e_outLocal0[(computeNum / 4)], computeNum / 4);
        ...
        if(cumulated_amount + outerNum >= computeNum){
            ...
            AIV_WITH_BARRIER(ShiftRight, mergeLocal[cumulated_amount], e_outLocal1, (uint16_t)4, low_write_num);
            // if(i == 0){
            ...
            //         , 1, 32);
            // }
            AIV_WITH_BARRIER(DataCopy, e_output1[write_offset], mergeLocal, (computeNum / 8));
            write_offset = write_offset + computeNum / 8;

            AIV_WITH_BARRIER(ShiftRight, mergeLocal, e_outLocal1[low_write_num], (uint16_t)4, high_unwrite_num);
            cumulated_amount = high_unwrite_num;
        }
        else {
            AIV_WITH_BARRIER(ShiftRight, mergeLocal[cumulated_amount], e_outLocal1, (uint16_t)4, outerNum);
            // if(i == 0){
            //     DumpTensor(mergeLocal[cumulated_amount], 1, outerNum);
            // }
    ...
    __aicore__ inline void CopyOut(uint32_t i)
    {
        ...
        AIV_WITH_BARRIER(DataCopy, e_output0[i * (computeNum / 4)], e_outLocal0, computeNum / 4);
