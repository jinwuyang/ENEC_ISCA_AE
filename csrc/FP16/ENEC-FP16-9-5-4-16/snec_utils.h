...
inline uint8_t *getMs0data(Header *cphd, uint8_t *compressed)// ms0: 低16位尾数（总共32位）,FP32专用
{
    return compressed + 32;
}

inline uint8_t *getMs1data(Header *cphd, uint8_t *compressed)// ms1: 符号位 + 高8位尾数（总共32位），三种数据类型都有
{
    if(cphd->dataType == 0 | cphd->dataType == 1)
        return getMs0data(cphd, compressed);
    else if (cphd->dataType == 2)
        return getMs0data(cphd, compressed) + cphd->totalUncompressedBytes / 2;
}

inline uint8_t *getEdata(Header *cphd, uint8_t *compressed)// 低位的指数
{
    if(cphd->dataType == 0)
        return getMs1data(cphd, compressed) + cphd->totalUncompressedBytes / sizeof(uint16_t) * 8 / 8;
    else if(cphd->dataType == 1)
        return getMs1data(cphd, compressed) + (cphd->dataBlockSize / sizeof(uint16_t)) * 11 / 8 * cphd->dataBlockNum;
    // cphd->totalUncompressedBytes / sizeof(uint16_t) * 11 / 8//有问题
    // ;
    else if (cphd->dataType == 2)
        return getMs1data(cphd, compressed) + cphd->totalUncompressedBytes / sizeof(uint32_t) * 8 / 8;
}
...
inline uint8_t *getMbl(Header *cphd, uint8_t *compressed)// compareMask
{
    if(cphd->dataType == 0 | cphd->dataType == 1)
        ...
        return getEdata(cphd, compressed) + (cphd->dataBlockSize / sizeof(uint32_t)) * 4 / 8 * cphd->dataBlockNum;
}
...
inline uint8_t *getCompSizePrefix(Header *cphd, uint8_t *compressed)// prefix
{
    if (cphd->dataType == 0 | cphd->dataType == 1)
        ...
        return getMbl(cphd, compressed) + cphd->dataBlockNum * (cphd->dataBlockSize / (cphd->tileLength * sizeof(uint32_t)) / 8);
}
...
inline int getFinalbufferSize(uint32_t byteSize, uint32_t tileNum, uint32_t DATA_BLOCK_BYTE_NUM_C) // 单位：byte
{
    ...
    int FinalBufferSize =   32 + // 头
                            ...
                            DATA_BLOCK_BYTE_NUM_C / sizeof(uint16_t) * 11 / 8 * datablockNum + // ms1
