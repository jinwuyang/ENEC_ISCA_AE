    ...
    __aicore__ inline void Process()
    {
        ...
        if (datatype == 0 | datatype == 1)
        {
            totalcompressedsize = 32 +
                                ...
                                datablocknum * (datablocksize / sizeof(uint16_t) * 11 / 8) + // ms1
                                datablocknum * (datablocksize / sizeof(uint16_t) * 3 / 8) + // e0
        ...
        else
        {
            totalcompressedsize = 32 +
                                totaluncompressedbytes / 2 +
                                totaluncompressedbytes / 4 +
                                datablocknum * (datablocksize / sizeof(uint32_t) * 3 / 8) +  
...
extern "C" void enec_merge(
    Header *cphd, 
    void *stream, 
    uint8_t *compressedDevice, 
    uint8_t *compressedFinal, 
    uint8_t *histogramDevice, 
    uint8_t *blockCompSizeDevice, 
    uint32_t bufferSize){
    merge<<<BLOCK_NUM, nullptr, stream>>>(
        compressedFinal, 
        ...
        getCompressed_exp(cphd, compressedFinal), 
        histogramDevice, 
        blockCompSizeDevice, 
        cphd->dataBlockSize, 
        cphd->dataBlockNum, 
